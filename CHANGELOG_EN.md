...
- feat: |Frontend| Support injecting the Google AdSense verification tag and Auto ads script into the build output `<head>` with only `VITE_GOOGLE_AD_CLIENT` configured, while keeping `VITE_GOOGLE_AD_SLOT` as the optional desktop sidebar manual ad-unit setting
- docs: |Docs| Expand the Google AdSense integration guide with site verification, frontend environment variables, build and deploy steps, `ads.txt` setup, and verification troubleshooting
