...
- feat: |Frontend| Google AdSense 支持仅配置 `VITE_GOOGLE_AD_CLIENT` 在构建产物 `<head>` 中注入验证标签与 Auto ads 脚本，并保留 `VITE_GOOGLE_AD_SLOT` 作为桌面侧栏手动广告位的可选配置
- docs: |文档| 完善 Google AdSense 接入流程文档，覆盖站点验证、前端环境变量、打包发布、`ads.txt` 配置和常见验证失败排障
